    ...
    this.sub = new Subscription({
      ...
      validate: (value: unknown) => {
        try {
          ...
          console.error('仓库订阅跳过，因消息无效。', err)
        }
      },
    })
  }
...
export const getOpsByType = async (evt: Commit): Promise<OperationsByType> => {
  ...
  for (const op of evt.ops) {
    ...
    if (op.action === 'update') continue // 目前不支持更新操作。
...
// @TODO 目前，对于 BlobRef 的记录验证出现了失败的情况。
// 仅仅是因为多个软件包都有各自独立的副本
// 在 BlobRef 类中，这会导致 instanceof 检查无法通过。
// 这是一个临时的解决方案。
